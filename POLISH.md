...
**Post-Fable (Opus 4.7)**:
- ✅ **Navbar overhaul cinemático** (`src/components/Navbar.astro` + `src/scripts/navbar.ts`): grid `1fr auto 1fr` — el hueco central del logo lo reserva el grid. Logo grande siempre (72px, sin shrink) que gira 1.5 vueltas `power2.inOut` al cruzar scroll >80px (CW abajo, CCW arriba). Ring SVG con `pathLength=1` alrededor del logo que se rellena según `scrollY/maxScroll`. `IntersectionObserver` con `rootMargin: '-30% 0px -60% 0px'` marca la sección activa (dot cyan a la izquierda del link matching). Kbd hints (`g p`, `g w`, `g t`, `g a`, `g u`, `g c`) aparecen bajo cada link al hover/focus. CTA `● open to work [ ↗ ]` en `data-magnetic` (reutiliza `initMagnetic`). Hairline cyan gradient dibujada de centro hacia los lados al entrar en compact. Altura barra 72→60px con transición.
